    ...
    func testSave() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            do {
                ...
                XCTAssertEqual(try Data(contentsOf: URL(string: "apple-touch-icon.png", relativeTo: url)!).count, 6971)
                XCTAssertEqual(try Data(contentsOf: URL(string: "favicon.ico", relativeTo: url)!).count, 1066)
                XCTAssertEqual(try Data(contentsOf: URL(string: "image.png", relativeTo: url)!).count, 48811)
